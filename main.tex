...
\newcommand\JoinUpF[4][10pt]{
\draw[<->]
  ([shift={#2}]pic cs:start#4) -- ++(0pt,#1) -| ([shift={(#3)}]pic cs:end#4);
}
\newcommand\JoinDownF[4][10pt]{
\draw[<->]
  ([shift={#2}]pic cs:start#4) -- ++(0pt,-#1) -| ([shift={(#3)}]pic cs:end#4);
}
...
\lowertitleback{
	...
	\url{https://it.overleaf.com/read/rjbjszztskbv}\\
	...
	\textbf{Publisher} \\
	First wrote in May 2022 by \@author\\
	
% 	Ahimè non c'è la stessa quantità di cazzate ed easter eggs che si può trovare negli appunti di teoria dei gruppi, stavolta è uscita una roba fin troppo seria. Se ci sono errori segnalateli e se vi sentite particolarmente generosi, io ve lo lascio qui che non si sa mai qualcuno impazzisca e voglia contribuire con un bel \href{https://www.paypal.me/mattderri}{PANINOZZO}.
% 	\\
% 	Sadly there is not the same quantity of funny things and easter eggs that you can find in group theory notes, this time it looks way too serious. If there is any mistake let me know and you feel particularly generous, I leave it here if someone wants to contribute with a big \href{https://www.paypal.me/mattderri}{PANINOZZO}.
}
\makeatother
...
%----------------------------------------------------------------------------------------
% \dedication{
%     Well, mister Bonciani, it is just a matter of understanding each other, of terminology. You say "on-shell states" and I say "states which satisfies the mass-shell relation", you say "$J^\mu$" and I say "$\pazocal{L}\delta x^\mu+\frac{\partial\pazocal{L}}{\partial\Phi_{i,\mu}}+\delta\Omega^\mu$", you say "$\partial_\mu\partial^\mu$" and I say "$\Box$". But overall, I agree with you.\\
% 	\begin{flushright} -- Galactic Mega Director, more or less.
% 	\end{flushright}
% 	\newline
% 	\small{Ma, caro Bonciani, è solo questione d'intendersi, di terminologia. Lei dice "stati on-shell" e io "stati che soddisfano la mass-shell relation", lei dice "$J^\mu$" e io dico "$\pazocal{L}\delta x^\mu+\frac{\partial\pazocal{L}}{\partial\Phi_{i,\mu}}+\delta\Omega^\mu$", lei dice "$\partial_\mu\partial^\mu$" e io "$\Box$". Ma per il resto, la penso esattamente come lei.}\\
% 	\begin{flushright} -- Mega Direttore Galattico, più o meno.
% 	\end{flushright}
% 	\newline
...
\part{Fields}
\subfile{Chapters/2_Classical_Fields}
\subfile{Chapters/3_KG_Field}
\subfile{Chapters/4_Dirac_Field}
\subfile{Chapters/5_EM_Field}
\part{Interactions}
\subfile{Chapters/6_Propagators}
\subfile{Chapters/7_EM_Interactions}
...
% \nomenclature{$\mathbb{H}$}{Spazio di Hilbert - \textit{Hilbert space}}
